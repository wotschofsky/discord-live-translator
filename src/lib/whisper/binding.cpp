...
Napi::String whisperFull(const Napi::CallbackInfo &napiInfo) {
  ...
  if (!napiInfo[0].IsString()) {
    Napi::TypeError::New(env, "First argument should be string")
        .ThrowAsJavaScriptException();
    return Napi::String::New(env, "Error");
  }
  ...
  if (!napiInfo[1].IsString()) {
    Napi::TypeError::New(env, "Second argument should be string")
        .ThrowAsJavaScriptException();
    return Napi::String::New(env, "Error");
  }
  ...
  if (!ctx) {
    Napi::TypeError::New(
        env, std::string("Failed to load whisper model: ") + model_path)
        .ThrowAsJavaScriptException();
    return Napi::String::New(env, "Error");
  }
  ...
  if (file == NULL) {
    Napi::TypeError::New(
        env, std::string("Failed to open file: ") + sf_strerror(file))
        .ThrowAsJavaScriptException();
    return Napi::String::New(env, "Error");
  }
  ...
  if (!samples) {
    Napi::TypeError::New(env, "Failed to allocate memory for samples")
        .ThrowAsJavaScriptException();
    sf_close(file);
    return Napi::String::New(env, "Error");
  }
  ...
  if (sf_read_float(file, samples, n_samples) != n_samples) {
    Napi::TypeError::New(
        env,
        std::string("Failed to read samples from file: ") + sf_strerror(file))
        .ThrowAsJavaScriptException();
    sf_close(file);
    delete[] samples;
    return Napi::String::New(env, "Error");
  ...
  if (result != 0) {
    Napi::TypeError::New(
        env, std::string("Failed to process audio: ") + std::to_string(result))
        .ThrowAsJavaScriptException();
    whisper_free(ctx);
    delete[] samples;
    return Napi::String::New(env, "Error");
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
  exports.Set(Napi::String::New(env, "whisperFull"),
